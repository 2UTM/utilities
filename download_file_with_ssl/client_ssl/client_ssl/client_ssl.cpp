...
int connectToServer(std::string ip, int port)
{
    ...
    if (err <= 0)
    {
        printf("Ошибка создания соединения SSL, %d\n", SSL_get_error(ssl, err));
        return 0;
    }
...
int RecvPacketUpdate(std::string& nameFile, std::string& sizeFile, long long& seek)
{
    ...
    if (RecvAll(ssl, &tmpSize.front(), 9))
    {
        printf("Ошибка recv: %d\n", WSAGetLastError());
        return 1;
    }
    ...
    if (RecvAll(ssl, &tmpNameFile.front(), 256))
    {
        printf("Ошибка recv: %d\n", WSAGetLastError());
        return 1;
    }
    ...
    if (RecvAll(ssl, &tmpSizeFile.front(), 12))
    {
        printf("Ошибка recv: %d\n", WSAGetLastError());
        return 1;
    }
